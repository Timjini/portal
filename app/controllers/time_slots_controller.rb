class TimeSlotsController < ApplicationController
  ...
  def create
    ...
    @calendar = CoachCalendar.where(user_id: current_user.id).first
    if @calendar.nil?
      @calendar = CoachCalendar.create(user_id: current_user.id)
    end
  ...
  private
    ...
    def time_slot_params
      params.require(:time_slot).permit(:coach_calendar_id, :date, :start_time, :end_time, :group_type)
    end
end
